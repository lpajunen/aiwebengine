...
4. **Standards alignment + a std-lib.** Non-standard globals mislead: `fetch` looks like WHATWG fetch but isn't; `scriptStorage`/`personalStorage` look like Web Storage but aren't; `URL`/`crypto.subtle`/`TextEncoder`/`structuredClone` appear absent. No npm at all (only asset-backed CommonJS-ish imports, no dynamic `import()`). Align the shapes with web standards and ship a small vetted std-lib of modules to offset the missing npm ecosystem.
