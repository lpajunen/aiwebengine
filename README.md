...
Undo a change you have no checkout for. Every write records a revision of the whole script — its root source and every file — so `GET /engine/revisions?script=<script>` reports what changed, when, at whose hand, and whether the script still initialised afterwards; `lastGood` names the newest revision that did. `check`, `run_tests` and `eval` take `revision=<n|head|last-good|label>`, so a version can be bundled and exercised without being deployed and a broken head does not make the version before it unreachable: see [What a Script's Files Have Been](docs/SCRIPT_REVISIONS.md).
