...
Edit a file without resending it. `PATCH /engine/assets?script=<script>&asset=<path>` applies string replacements to an asset the engine already has, refusing an ambiguous match and — when the caller passes the `base_sha256` it read — a version that has moved on since. `POST /engine/edit_script` does the same for the one file of a script that is not an asset, its root source, under what writing a script takes rather than what writing an asset takes. `GET /engine/assets` reads back the same way: `lines=120-180` for a range, `grep=<pattern>` to locate without downloading; `GET /engine/read_script` reports the digest to aim an edit with as its `ETag`: see [Editing a Script's Files Without Resending Them](docs/ASSET_EDIT.md).
