...
Inspect a running solution without deploying anything to do it. `POST /engine/eval?uri=<script>` evaluates a snippet against a deployed script's sandbox — its own functions and imported modules in scope — and returns the value plus everything it logged, rolling back database writes by default: see [Evaluating Snippets Against a Script](docs/SCRIPT_EVAL.md).
...
Edit a file without resending it. `PATCH /engine/assets?script=<script>&asset=<path>` applies string replacements to an asset the engine already has, refusing an ambiguous match and — when the caller passes the `base_sha256` it read — a version that has moved on since. `POST /engine/edit_script` does the same for the one file of a script that is not an asset, its root source, under what writing a script takes rather than what writing an asset takes. `GET /engine/assets` and `GET /engine/read_script` read back the same way: `lines=120-180` for a range, `grep=<pattern>` to locate without downloading, and the digest to aim the next edit with reported either way. `GET /engine/search?query=<pattern>` is the step before that — which file mentions this — across every script's root source and modules: see [Editing a Script's Files Without Resending Them](docs/ASSET_EDIT.md).

Choose what production serves. `POST /engine/deploy?script=<script>&revision=<n|head|last-good|label>` pins a script to a revision: writing its files then records revisions and advances head without changing what answers requests, so code can be uploaded, checked and tested while production stays where it is and moves only on command. `GET /engine/deploy` reports what is served, what is newest, and how far apart they are; `DELETE` goes back to following head: see [What a Script's Files Have Been](docs/SCRIPT_REVISIONS.md).

Undo a change you have no checkout for. Every write records a revision of the whole script — its root source and every file — so `GET /engine/revisions?script=<script>` reports what changed, when, at whose hand, and whether the script still initialised afterwards; `lastGood` names the newest revision that did. `check`, `run_tests` and `eval` take `revision=<n|head|last-good|label>`, so a version can be bundled and exercised without being deployed and a broken head does not make the version before it unreachable. `POST /engine/revisions/revert` puts the files back as a new revision rather than a rewrite of history, removing what the target never had, refusing a target that does not bundle, and reporting the change first with `dry_run`. `GET /engine/revisions/diff` shows what changed as a unified diff per file — by default the newest change — and `POST /engine/revisions/label` names a revision so it can be restored by name and kept through retention: see [What a Script's Files Have Been](docs/SCRIPT_REVISIONS.md).

Debug a running solution by what it says. Every line a script logs records the invocation that emitted it and the revision that was running, so `GET /engine/script_logs?request_id=<id>` returns exactly the lines one request, scheduler tick or message listener produced, `revision=<n>` returns everything one version produced, and `route=`, `kind=`, `contains=` and a `seq` cursor narrow the rest. `GET /engine/script_logs/stream` follows the same, filtered, as it is written — a live tail of a real session: see [Reading a Script's Log](docs/SCRIPT_LOGS.md).
...
There used to be a third tier between them, for APIs one script could offer another inside the same engine. Nothing implements it any more: what enforced it was a `visibility` argument on the GraphQL registry, and the one API a script could actually reach another script through was the message dispatcher. Both are gone. A script now reaches another script's work only through the engine's own management tools, and those are authorized against the calling user rather than against the fact that both scripts happen to run here.
