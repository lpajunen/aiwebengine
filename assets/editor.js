...
class AIWebEngineEditor {
    ...
    compileTemplates() {
        const templateIds = ['script-item-template', 'asset-item-template', 'log-entry-template', 'route-item-template'];

        templateIds.forEach(id => {
            const templateElement = document.getElementById(id);
            if (templateElement) {
                this.templates[id.replace('-template', '')] = Handlebars.compile(templateElement.innerHTML);
            }
        });
    }
    ...
    setupEventListeners() {
        ...
        // Asset management
        document.getElementById('upload-asset-btn').addEventListener('click', () => this.triggerAssetUpload());
        document.getElementById('asset-upload').addEventListener('change', (e) => this.uploadAssets(e.target.files));
        ...
        // Test endpoint
        document.getElementById('test-endpoint-btn').addEventListener('click', () => this.testEndpoint());
    }
    ...
    async loadScripts() {
        try {
            ...
            scripts.forEach(script => {
                ...
                scriptElement.querySelector('.script-item').addEventListener('click', () => {
    ...
    createNewScript() {
        ...
        const fullName = scriptName.endsWith('.js') ? scriptName : scriptName + '.js';
...
register('/', 'handler', 'GET');`
    ...
    deleteCurrentScript() {
        ...
        if (!confirm(`Are you sure you want to delete ${this.currentScript}?`)) return;
    ...
    async loadRoutes() {
        try {
            ...
            routesList.innerHTML = '<div class="loading">Routes listing not yet implemented</div>';
        } catch (error) {
