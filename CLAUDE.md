...
- Built-in engine scripts live in `scripts/feature_scripts/` (`core.js`, `auth.js`) and are bootstrapped into the DB at startup; ad hoc test fixtures live in `scripts/test_scripts/`. Script/asset management REST endpoints and the engine MCP tools are implemented natively in Rust (`src/engine_api.rs`), not in the built-in scripts. Example/demo solution scripts live in the separate [aiwebengine-examples](https://github.com/lpajunen/aiwebengine-examples) repo, not in this repo.
