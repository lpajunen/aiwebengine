...
- `server.management_hosts` restricts every `/engine/*` endpoint except the static `/engine/installed` page to the listed hosts; empty means all hosts. Enforced twice: a layer on the management router in `lib.rs`, and a `/engine/` prefix check in `handle_dynamic_request` that covers the `/engine/script_updates` stream.
