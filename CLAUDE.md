...
- All scripts are equal — there is no privileged/restricted split. Every script sees the same engine-internal API surface, and each call is authorized against the _calling user_: their capabilities, whether they own the target script (`script_owners` table, `repository::user_owns_script`), and whether they hold the editor/administrator role (`user_repository::UserRole`). Cross-script operations (`assetStorage.*ForUri`, `secretStorage.*ForUri`, ownership changes) require the relevant capability, ownership, or admin rights.
