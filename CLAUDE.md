...
- Built-in engine scripts live in `scripts/feature_scripts/` (`core.js`, `auth.js`, `admin.js`, `cli.js`) and are bootstrapped into the DB at startup; ad hoc test fixtures live in `scripts/test_scripts/`. Example/demo solution scripts live in the separate [aiwebengine-examples](https://github.com/lpajunen/aiwebengine-examples) repo, not in this repo.
