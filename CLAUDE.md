...
- The only built-in engine script is `scripts/feature_scripts/core.js` (public asset routes, the `/script_updates` stream, heartbeat jobs), bootstrapped into the DB at startup; ad hoc test fixtures live in `scripts/test_scripts/`. All other engine endpoints — script/asset management REST, the engine MCP tools, `/engine/installed`, `/engine/openapi.json`, `/auth/unauthorized` — are implemented natively in Rust (`src/engine_api.rs`). Example/demo solution scripts live in the separate [aiwebengine-examples](https://github.com/lpajunen/aiwebengine-examples) repo, not in this repo.
