...
fn test_manager_script_loads() {
    ...
    assert!(script_content.contains("function init("), "Manager script should have init function");
    assert!(script_content.contains("handleManagerUI"), "Manager script should have handleManagerUI function");
    assert!(script_content.contains("handleListUsers"), "Manager script should have handleListUsers function");
    assert!(script_content.contains("handleUpdateUserRole"), "Manager script should have handleUpdateUserRole function");
}
...
fn test_manager_script_executes() {
    ...
    let script_content = repository::fetch_script(script_uri)
        .expect("Manager script should be in repository");
    ...
    assert!(result.success, "Manager script should execute successfully: {:?}", result.error);
}
...
fn test_manager_script_init() {
    ...
    assert!(registrations.is_ok(), "Manager script init should succeed: {:?}", registrations.err());
    let registrations = registrations.unwrap();
    assert!(registrations.is_some(), "Manager script should have init function");
