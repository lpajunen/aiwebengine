...
impl AppleProvider {
    ...
    fn generate_client_secret(&self) -> Result<String, AuthError> {
        ...
        let encoding_key = EncodingKey::from_ec_pem(self.private_key.as_bytes()).map_err(|e| {
            AuthError::JwtError(format!("Failed to parse Apple private key: {}", e))
        })?;
...
impl OAuth2Provider for AppleProvider {
    ...
    async fn exchange_code(
        ...
        let token_response: AppleTokenResponseRaw = response.json().await.map_err(|e| {
