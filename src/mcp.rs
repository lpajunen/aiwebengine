...
/// Registry for storing MCP tools registered from JavaScript
...
impl McpRegistry {
    ...
    pub fn clear_script_registrations(&mut self, script_uri: &str) {
        debug!("Clearing MCP tool registrations for script: {}", script_uri);
...
/// Clear all MCP tool registrations from a specific script URI
pub fn clear_script_mcp_registrations(script_uri: &str) {
    debug!("Clearing MCP tool registrations for script: {}", script_uri);
    if let Ok(mut registry) = get_registry().write() {
        registry.clear_script_registrations(script_uri);
        debug!(
            "Successfully cleared MCP tool registrations for script: {}",
            script_uri
        );
    } else {
